...
class Model_Files {
  static async getAll() {
    return new Promise((resolve, reject) => {
      connection.query(
        "SELECT file.*, users.*, kategori.*, COUNT(file.id_file) as jumlah_file FROM file JOIN users ON file.id_user = users.id_user JOIN kategori ON file.id_kategori = kategori.id_kategori WHERE file.pengajuan = 'setuju' GROUP BY file.id_file",
        (err, rows) => {
          if (err) {
            reject(err);
          } else {
