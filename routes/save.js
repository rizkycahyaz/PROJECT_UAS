...
router.get("/", async function (req, res, next) {
  ...
  try {
    if (Data.length > 0) {
      let rows = await Model_Save.getAllByUser(id);
...
router.get("/create", async function (req, res, next) {
  try {
    ...
    let saveData = await Model_Save.getAllByUser(id); // Mengambil data yang disimpan oleh user
...
router.get("/delete/(:id)", async function (req, res) {
  try {
    ...
    if (Data[0].role == 1) {
      ...
      req.flash("error", "File dihapus dari daftar simpan");
      res.redirect("/save");
